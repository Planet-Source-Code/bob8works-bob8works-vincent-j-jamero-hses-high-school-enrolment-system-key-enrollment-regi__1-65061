...
Begin VB.Form PickGraduate 
   ...
   Begin MSComctlLib.ListView listRecord 
      ...
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      MouseIcon       =   "PickGraduate.frx":058A
      NumItems        =   3
   ...
   Begin MSComctlLib.ImageList ilRecordIco 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "PickGraduate.frx":1538
            Key             =   ""
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "PickGraduate.frx":1AD2
            Key             =   ""
         EndProperty
         BeginProperty ListImage3 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "PickGraduate.frx":206C
            Key             =   ""
         EndProperty
         BeginProperty ListImage4 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "PickGraduate.frx":2606
            Key             =   ""
         EndProperty
      EndProperty
   End
End
Attribute VB_Name = "PickGraduate"
...
Dim curStudentFullName


Public Function GetStudentID(Optional ByRef TextObject As Object, Optional ByRef sStudentFullName As String, Optional lMaxEntryCount As Long = 15) As String
    ...
    sSQL = "SELECT tblStudent.StudentID AS lvKey, tblStudent.StudentID, [tblStudent].[LastName] & ', ' & [tblStudent].[FirstName] & ' ' & [tblStudent].[MiddleName] AS StudentFullName, Avg(tblGrade.GradeValue) AS AvgOfGradeValue" & _
            " FROM (tblSection INNER JOIN (tblSectionOffering INNER JOIN (tblStudent INNER JOIN tblEnrolment ON tblStudent.StudentID = tblEnrolment.StudentID) ON tblSectionOffering.SectionOfferingID = tblEnrolment.SectionOfferingID) ON tblSection.SectionID = tblSectionOffering.SectionID) INNER JOIN tblGrade ON tblEnrolment.EnrolmentID = tblGrade.EnrolmentID" & _
            " Where (((tblSection.YearLevelID) = 4))" & _
            " GROUP BY tblStudent.StudentID, tblStudent.StudentID, [tblStudent].[LastName] & ', ' & [tblStudent].[FirstName] & ' ' & [tblStudent].[MiddleName]" & _
            " HAVING (((Avg(tblGrade.GradeValue))>75));"
    'add yr to list
    If ConnectRS(HSESDB, vRS, sSQL) = True Then
        
        b8navRecordRefresh
        
        If CurStudentCount > 0 Then
            FillList CurRecPos, MaxEntryCount

        Else
            'error
            MsgBox "No Student Account to be selected." & vbNewLine & "Please Add New Student Account first.", vbExclamation
            Unload Me
            Exit Function
        End If
    Else
...
Private Sub ReturnGetStudentID()
    If Len(GetLVKey(listRecord.SelectedItem)) > 0 Then
        tmpStudentID = GetLVKey(listRecord.SelectedItem)
        curStudentFullName = listRecord.SelectedItem.SubItems(1)
        Unload Me
    End If
End Sub
...
Private Sub cmdFind_Click()
   ...
   sSQL = "SELECT tblStudent.StudentID AS lvKey, tblStudent.StudentID, [tblStudent].[LastName] & ', ' & [tblStudent].[FirstName] & ' ' & [tblStudent].[MiddleName] AS StudentFullName, Avg(tblGrade.GradeValue) AS AvgOfGradeValue" & _
            " FROM (tblSection INNER JOIN (tblSectionOffering INNER JOIN (tblStudent INNER JOIN tblEnrolment ON tblStudent.StudentID = tblEnrolment.StudentID) ON tblSectionOffering.SectionOfferingID = tblEnrolment.SectionOfferingID) ON tblSection.SectionID = tblSectionOffering.SectionID) INNER JOIN tblGrade ON tblEnrolment.EnrolmentID = tblGrade.EnrolmentID" & _
            " Where (((tblSection.YearLevelID) = 4) AND (([tblStudent]![LastName]+', '+[tblStudent]![FirstName]+' '+[tblStudent]![MiddleName]) like '%" & txtFind.Text & "%') )" & _
            " GROUP BY tblStudent.StudentID, tblStudent.StudentID, [tblStudent].[LastName] & ', ' & [tblStudent].[FirstName] & ' ' & [tblStudent].[MiddleName]" & _
            " HAVING (((Avg(tblGrade.GradeValue))>75));"
