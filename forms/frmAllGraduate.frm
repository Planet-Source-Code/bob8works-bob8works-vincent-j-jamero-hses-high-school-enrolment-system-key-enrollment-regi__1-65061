...
Begin VB.Form frmAllGraduate 
   ...
   ClientWidth     =   7560
   ControlBox      =   0   'False
   Icon            =   "frmAllGraduate.frx":0000
   LinkTopic       =   "Form1"
   MDIChild        =   -1  'True
   Picture         =   "frmAllGraduate.frx":058A
   ScaleHeight     =   424
   ScaleMode       =   3  'Pixel
   ScaleWidth      =   504
   Begin VB.PictureBox bgMain 
      ...
      Begin MSComctlLib.ImageList ilRecordIco 
         ...
         BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
            ...
            BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmAllGraduate.frx":0E54
               Key             =   ""
            EndProperty
         EndProperty
      End
      Begin MSComctlLib.ImageList icoHeader 
         ...
         BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
            NumListImages   =   2
            BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmAllGraduate.frx":13EE
               Key             =   ""
            EndProperty
            BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmAllGraduate.frx":1988
               Key             =   ""
            EndProperty
         EndProperty
      End
   ...
   End
End
Attribute VB_Name = "frmAllGraduate"
...
Public Sub ShowFormList(Optional iMaxEntryCount As Long = 19, Optional iCurRecPos As Long = 0)
    ...
    'set default SQL
    sDefaultSQL = "SELECT tblGraduate.StudentID, tblGraduate.SchoolYear, tblDepartment.DepartmentTitle, tblSection.SectionTitle, [tblStudent].[LastName] & ', ' & [tblStudent].[FirstName] & ' ' & [tblStudent].[MiddleName] AS StudentFullName, Avg(tblGrade.GradeValue) AS AvgOfGradeValue" & _
                    " FROM tblDepartment INNER JOIN ((tblSection INNER JOIN tblSectionOffering ON tblSection.SectionID = tblSectionOffering.SectionID) INNER JOIN (tblGraduate INNER JOIN (tblStudent INNER JOIN (tblEnrolment INNER JOIN tblGrade ON tblEnrolment.EnrolmentID = tblGrade.EnrolmentID) ON tblStudent.StudentID = tblEnrolment.StudentID) ON tblGraduate.StudentID = tblStudent.StudentID) ON tblSectionOffering.SectionOfferingID = tblEnrolment.SectionOfferingID) ON tblDepartment.DepartmentID = tblSection.DepartmentID" & _
                    " GROUP BY tblGraduate.StudentID, tblGraduate.SchoolYear, tblDepartment.DepartmentTitle, tblSection.SectionTitle, [tblStudent].[LastName] & ', ' & [tblStudent].[FirstName] & ' ' & [tblStudent].[MiddleName], tblSection.YearLevelID" & _
                    " Having (((tblSection.YearLevelID) = 4))" & _
                    " ORDER BY tblGraduate.SchoolYear DESC;"
        
    'set SQL
    sSQL = "SELECT tblGraduate.StudentID, tblGraduate.SchoolYear, tblDepartment.DepartmentTitle, tblSection.SectionTitle, [tblStudent].[LastName] & ', ' & [tblStudent].[FirstName] & ' ' & [tblStudent].[MiddleName] AS StudentFullName, Avg(tblGrade.GradeValue) AS AvgOfGradeValue" & _
                    " FROM tblDepartment INNER JOIN ((tblSection INNER JOIN tblSectionOffering ON tblSection.SectionID = tblSectionOffering.SectionID) INNER JOIN (tblGraduate INNER JOIN (tblStudent INNER JOIN (tblEnrolment INNER JOIN tblGrade ON tblEnrolment.EnrolmentID = tblGrade.EnrolmentID) ON tblStudent.StudentID = tblEnrolment.StudentID) ON tblGraduate.StudentID = tblStudent.StudentID) ON tblSectionOffering.SectionOfferingID = tblEnrolment.SectionOfferingID) ON tblDepartment.DepartmentID = tblSection.DepartmentID" & _
                    " GROUP BY tblGraduate.StudentID, tblGraduate.SchoolYear, tblDepartment.DepartmentTitle, tblSection.SectionTitle, [tblStudent].[LastName] & ', ' & [tblStudent].[FirstName] & ' ' & [tblStudent].[MiddleName], tblSection.YearLevelID" & _
                    " Having (((tblSection.YearLevelID) = 4))" & _
                    " ORDER BY tblGraduate.SchoolYear DESC;"
...
Public Function Form_Add()
    If frmAddGraduate.ShowForm = True Then
        Form_Reload
    End If
End Function
...
Public Sub Form_Delete()
    If Len(listRecord.SelectedItem.Text) > 0 Then
        If frmDeleteGraduate.ShowForm(GetLVKey(listRecord.SelectedItem), listRecord.SelectedItem.Text) = True Then
            Form_Reload
        End If
    Else
...
Public Function Form_Filter(sFindWhat As String, sFieldName As String)
    ...
    If Form_OrigFields(FIndex) = sAllFields Then
        ...
        sSQL = sSQL & " GROUP BY tblGraduate.StudentID, tblGraduate.SchoolYear, tblDepartment.DepartmentTitle, tblSection.SectionTitle, [tblStudent].[LastName] & ', ' & [tblStudent].[FirstName] & ' ' & [tblStudent].[MiddleName], tblSection.YearLevelID" & _
                    " Having (((tblSection.YearLevelID) = 4))" & _
                    " ORDER BY tblGraduate.SchoolYear DESC;"
...
Private Function Form_SetFieldList()
    ...
    Form_OrigFields(4) = "[tblStudent].[LastName] & ', ' & [tblStudent].[FirstName] & ' ' & [tblStudent].[MiddleName]"
    Form_Fields(4) = "Student Full Name"
    
    ReDim Preserve Form_OrigFields(4) As String
    ReDim Preserve Form_Fields(4) As String
...
Public Function Form_Print()
    ...
    Set drGraduates.DataSource = vRS
    
        drGraduates.Show vbModal
End Function
